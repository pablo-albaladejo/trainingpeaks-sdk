  ...
  (): WorkoutValidationService => ({
    validateStructuredWorkoutBusinessRules: (
      ...
      if (athleteId === null || athleteId === undefined) {
        throw new InvalidAthleteError('Athlete ID cannot be null or undefined');
      }
      ...
      if (!structure) {
        throw new InvalidWorkoutStructureError('Workout structure is required');
      }
    ...
    validateListWorkoutsFilters: (filters: {
      ...
      if (filters.activityType !== undefined && filters.activityType !== null) {
      ...
      // Validate tags
      if (filters.tags && filters.tags.length > WORKOUT_LIMITS.TAGS_MAX_COUNT) {
        throw new InvalidWorkoutFiltersError(
